...
class UViT(nn.Module):
    ...
    def __init__(
        ...
        # Positional embedding for patches (learnable is faster than sinusoidal)
        ...
        # Transformer blocks - split into encoder (shallow) and decoder (deep)
        self.num_encoder_layers = depth // 2
        self.num_decoder_layers = depth // 2
        
        # Encoder blocks (shallow)
        ...
        # This helps preserve low-level details from the input
        # Project concatenated features (embed_dim from transformer + embed_dim from input)
        self.long_skip_proj = nn.Linear(embed_dim * 2, embed_dim)
        
        # Unpatchify: Convert patches back to image
        # Use the same stride as patchify for proper reconstruction
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        # Patchify: (batch, channels, H, W) -> (batch, embed_dim, H', W')
        x = self.patchify(x)
        
        # Rearrange to sequence: (batch, embed_dim, H', W') -> (batch, num_patches, embed_dim)
        # Using flatten + transpose is faster than einops rearrange for simple cases
        x = x.flatten(2).transpose(1, 2)  # (batch, num_patches, embed_dim)
        
        # Store input patches for long skip connection (preserves low-level details)
        ...
        # Concatenate time token: (batch, num_patches + 1, embed_dim)
        ...
        x = x.transpose(1, 2).reshape(batch_size, self.embed_dim, self.num_patches_h, self.num_patches_w)
