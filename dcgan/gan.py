...
class DCGAN:
    def __init__(self, pretrained=True):
        ...
        self.car_img_dir = os.path.join(self.base_dir, "car")
        self.car_output_dir = os.path.join(self.base_dir, "car")
        os.makedirs(self.car_output_dir, exist_ok=True)
        ...
        if pretrained:
            self.discriminator = load_model(os.path.join(self.models_dir, "vroumdis2.h5"))
            self.generator = load_model(os.path.join(self.models_dir, "vroumgen2.h5"))
    ...
    def train(self, batch_size=128, save_interval=50, save_img_interval=50):
        ...
        # get dataset
        X_train = self.load_dataset(os.path.join(self.car_img_dir, "*"))
        ...
        while 1:
            ...
            if epoch % save_interval == 0:
                ...
                # self.generator.save(os.path.join(self.models_dir, f'vroumgen_{epoch}.h5'))
                # self.discriminator.save(os.path.join(self.models_dir, f'vroumdis_{epoch}.h5'))

                self.generator.save(os.path.join(self.models_dir, "vroumgen2.h5"))
                self.discriminator.save(os.path.join(self.models_dir, "vroumdis2.h5"))
    ...
    def save_imgs(self, e):
        ...
        output_path = os.path.join(self.car_output_dir, f"{time.time()}_{e}.png")
        cv2.imwrite(output_path, gen_img[0])

    def load_dataset(self, path):

        try:
            # try to load existing X_train
            dataset_path = os.path.join(self.base_dir, "X_train.npy")
